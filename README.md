...
The golden demo path is: start in mock voice mode, paste and confirm a practice FAQ (a trading or SaaS FAQ works as well as e-commerce), pick a suggested drill from the "From your FAQ" group (or a built-in drill), allow microphone access, speak to the simulated customer, then review the four-score coaching report. Suggested drills are derived from the confirmed FAQ's own sections, so practice and scoring always match your material's domain.
