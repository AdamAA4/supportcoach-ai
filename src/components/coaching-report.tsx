...
function FeedbackList({ title, items, empty }: { title: string; items: string[]; empty: string }) {
  return (
    <section className="rounded-2xl bg-shell p-1.5 ring-1 ring-line">
    ...
    <main className="mx-auto w-full max-w-6xl px-5 py-10 sm:py-14">
      <div className="space-y-5 sm:space-y-6">
        <header className="settle-in">
          <h1 className="text-4xl font-bold leading-[1.08] tracking-[-0.025em] text-balance text-ink sm:text-[2.75rem]">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {dimensions.map(({ key, label, explanation }, index) => (
            <section key={key} className={`settle-in rounded-2xl bg-shell p-1.5 ring-1 ring-line ${scoreDelays[index]}`}>
              <div className="rounded-xl bg-panel p-5">
                <h2 className="text-sm font-bold text-ink">{label}</h2>
                <p className="mt-3 font-mono text-4xl font-bold tabular-nums text-ink" aria-label={`${label}: ${report.scores[key]} out of 3`}>
                  {report.scores[key]}
                  <span className="text-base font-medium text-ink-muted"> / 3</span>
                </p>
                <div className="mt-3 flex gap-1" aria-hidden="true">
                  {[0, 1, 2].map((step) => (
                    <span key={step} className={`h-1.5 flex-1 rounded-full ${step < report.scores[key] ? "bg-amber" : "bg-line"}`} />
                  ))}
                </div>
        ...
        <section className="rounded-2xl bg-shell p-1.5 ring-1 ring-amber/50">
          <div className="rounded-xl bg-warn-bg p-5">
            <h2 className="text-lg font-bold tracking-tight text-warn-ink">Next exercise</h2>
            <p className="mt-3 break-words leading-relaxed text-ink-soft">{report.nextExercise}</p>
          </div>
        </section>
        <details className="group rounded-xl bg-panel p-5 ring-1 ring-line">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-3 font-bold text-ink [&::-webkit-details-marker]:hidden">
            Full transcript
            <ChevronDownIcon className="size-4 shrink-0 text-ink-muted transition-transform duration-150 group-open:rotate-180" />
          </summary>
          <div className="mt-4"><TranscriptPane turns={report.transcript} /></div>
        </details>
        <p className="break-all font-mono text-xs leading-relaxed text-ink-muted">
