...
export function CallConsole({ context, createAgent = createMockAgent }: CallConsoleProps) {
  ...
  const handleEvent = useCallback((event: VoiceAgentEvent) => {
    ...
    if (event.type === "error") { setError(event.message); setCustomerAudioActive(false); transition(event); }
  }, [append, transition]);
  ...
  const startMicrophone = async () => {
    try { await agent.current?.startMicrophone(); setMicrophone("recording"); }
    catch { setMicrophone("typed-fallback"); }
  };
  const mute = () => { setMicrophone((current) => current === "muted" ? "not-started" : "muted"); };
  const sendTyped = () => { const text = typedText.trim(); if (!text || state === "error") return; agent.current?.sendTypedTraineeTurn(text); setTypedText(""); setMicrophone("typed-fallback"); };
